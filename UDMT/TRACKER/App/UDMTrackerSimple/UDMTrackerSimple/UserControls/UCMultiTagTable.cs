...
{
    public partial class UCMultiTagTable : DevExpress.XtraEditors.XtraUserControl
    {
        protected bool m_bEditable = true;
        protected CPlcLogicData m_cPlcLogicData = null;

        public event UEventHandlerMultiTagTableInputTextRequest UEventInputTextRequest;

        public event UEventHandlerMultiTagTableTagAdded UEventTagAdded;
        public event UEventHandlerMultiTagTableTagRemoved UEventTagRemoved;
        public event UEventHandlerMultiTagTableTagUpdated UEventTagUpdated;
        public event UEventHandlerMultiTagTableTagDoubleClicked UEventTagDoubleClicked;

        protected bool m_bDragDropReady = false;

        public UCMultiTagTable()
        {
            InitializeComponent();
        }
        ...
        public void ShowTable()
        {
            ...
            if (m_cPlcLogicData == null || m_cPlcLogicData.PlcName == "")
                return;

            if (m_cPlcLogicData != null)
            {
                for (int i = 0; i < CMultiProject.PlcProcS.Count; i++)
                    exEditorGroupCombo.Items.Add(CMultiProject.PlcProcS.ElementAt(i).Value.Name);
            }

            exGridMain.BeginUpdate();
            {
                List<CTag> lstViewTagS = new List<CTag>();
                lstViewTagS.AddRange(m_cPlcLogicData.TagS.Values);
        ...
        protected void SetData(CPlcLogicData cData)
        {
            m_cPlcLogicData = cData;
            ShowTable();
        }
        ...
        private void exGridMain_DragDrop(object sender, DragEventArgs e)
        {
            ...
            if (e.Data != null && e.Data.GetDataPresent(typeof(CTagS)))
            {
                ...
                if (cTagS != null)
                {
                    ...
                    {
                        cTag = cTagS[i];
                        if (m_cPlcLogicData.TagS.ContainsKey(cTag.Key) == false)
                        {
                            m_cPlcLogicData.TagS.Add(cTag.Key, cTag);
