...
namespace UDMTrackerSimple
{
    ...
    public partial class UCStepLadderView : UserControl
    {
                    ...
                    foreach (CPlcLogicData cPlcLogic in CMultiProject.PlcLogicDataS.Values)
                    {
                        ...
                        foreach (IGrouping<string, CStep> lstProgram in lstPrograms)
                        {
                            ...
                            foreach (CStep cStep in lstProgramSteps)
                            {
                                ...
                                trnStepNodeAll = CreateTreeNode(trnPrgNodeAll, cStep.Key, cStep.Description, IMG_INDEX_STEPS, cStep);

                                //if (cStep.FBInfoList.Count > 0)
                                //{
                                //    trnStepNodeAll = CreateTreeNode(trnPrgNodeAll, cStep.Key, cStep.Description, IMG_INDEX_STEPS, cStep);

                                //    //foreach(CFB_Info cFB in cStep.FBInfoList)
                                //    //{
                                //    //    if(cFB.InOut_ItemNameList.Count > 0)
                                //    //    {
                                //    //        //trnStepNodeAll = CreateTreeNode(trnPrgNodeAll, cStep.Key, cStep.Description, IMG_INDEX_STEPS, cStep);
                                //    //    }
                                //    //}
        ...
        private void SetLadderStep(CStep cStep, int iStepLevel, bool bView)
        {
            ...
            try
            {
                if (cStep != null)
                {
                    ...
                    CTag cLadderTag = null;

                    if(cLadderCoil.ContentS.Count > 0)
                        cLadderTag = cLadderCoil.ContentS[0].Tag;
                    ...
                    if (cLadderTag != null)
                    {
                        ucStep.StepName =
                           string.Format("CPU : {0} / Program : {1} / Network : {2} / Coil : {3} ( {4} )",
                               cLadderTag.Channel, cStep.Program, cStep.StepIndex, cStep.Address, cStep.Description);
                    }
                    else
                    {
                        ucStep.StepName =
        ...
        private void exTreeStep_MouseDoubleClick(object sender, MouseEventArgs e)
        {
            try
            {
                ...
                if (trnNode != null && trnNode.Tag != null)
                {
                    ...
                    tp.Name = "tp" + cStep.Key;
                    tp.Text = cStep.Key + " : " + cStep.Description;
                    tp.Tag = cStep;
        ...
        {
            FrmLadderLogViewer frmLadderLogViewer = new FrmLadderLogViewer();
            frmLadderLogViewer.ShowDialog();
        }
        #endregion
