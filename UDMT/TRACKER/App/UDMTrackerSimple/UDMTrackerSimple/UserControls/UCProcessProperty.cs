...
namespace UDMTrackerSimple
{
                ...
                if (e.Data != null)
                {
                    if (e.Data.GetDataPresent(typeof(CTagS)))
                    {
                        ...
                        SplashScreenManager.ShowDefaultWaitForm();
                        {
                            foreach (var who in cTagS)
                            {
                                if (m_cProcess.KeySymbolS.ContainsKey(who.Key))
                                    continue;

                                if (CheckPlcOutputDevice(who.Value))
                                {
                                    CKeySymbol cSymbol = new CKeySymbol(who.Value);
                                    m_cProcess.KeySymbolS.Add(who.Key, cSymbol);

                                    if(m_cProcess.ChartViewTagS == null)
                                        m_cProcess.ChartViewTagS = new CTagS();

                                    if (!m_cProcess.ChartViewTagS.ContainsKey(who.Key))
                                        m_cProcess.ChartViewTagS.Add(who.Key, who.Value);
                                }
                                else
    ...
    "Key Symbol Add", MessageBoxButtons.OK, MessageBoxIcon.Error);
            ...
            {
                Console.WriteLine("Process Property KeySymbolS Drag Error : " + string.Format("Method : {0}, Message : {1}", System.Reflection.MethodBase.GetCurrentMethod().Name, ex.Message));
                ex.Data.Clear();
            }
        }
        ...
        private void mnuClear_Click(object sender, EventArgs e)
        {
            ...
            else if (cntxMenu.SourceControl == grdKeySymbolS)
            {
                DialogResult dlgResult = XtraMessageBox.Show("Key Symbol을 모두 지우시겠습니까?", "Key Symbol Clear",
                    MessageBoxButtons.YesNo, MessageBoxIcon.Question);
        ...
        private void mnuDelete_Click(object sender, EventArgs e)
        {
            ...
            {
                DialogResult dlgResult = XtraMessageBox.Show("선택하신 Key Symbol을 제거하시겠습니까?", "Key Symbol Delete", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
                ...
                int[] arrRows = grvKeySymbolS.GetSelectedRows();

                foreach (int iRowHandle in arrRows)
                {
                    if (grvKeySymbolS.GetRow(iRowHandle).GetType() != typeof (CTag))
                        continue;

                    cTag = (CTag) grvKeySymbolS.GetRow(iRowHandle);

                    if (cTag == null)
                        continue;

                    if (m_cProcess.KeySymbolS.ContainsKey(cTag.Key))
                        m_cProcess.KeySymbolS.Remove(cTag.Key);
                }
