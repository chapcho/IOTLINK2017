...
namespace UDM.EnergyDaq.Monitor
{
    public class CModbusSerialPortConnecter
    {
        ...
        protected Queue<CSerialDeviceConfig> m_qDeviceConfig = new Queue<CSerialDeviceConfig>();
        protected CSerialDeviceConfig m_cCurrentDevice = null;
        protected CMeterDataRecorder m_cMeterDataRecorder = null;
        ...
        public CMeterDataRecorder DataRecorder
        {
            get { return m_cMeterDataRecorder; }
            set { m_cMeterDataRecorder = value; }
        }
        ...
        public bool SeriaPortDataSend(byte[] bDataS,CSerialDeviceConfig tempDivice)
        {
            ...
            if (m_IOSerialPort.IsOpen)
            {
                ...
                m_qDeviceConfig.Enqueue(tempDivice);
                return true;
            }
            else
            {
        ...
        private void ReceivedDataAnalysis(byte[] DataS)
        {
            try
            {
                ...
                if (bCRC[0] == bCRCl && bCRC[1] == bCRCh)
                {
                    ...
                    cData.DeviceInfo = m_cCurrentDevice.MeterDeviceKey;
                    cData.DeviceModel = m_cCurrentDevice.MeterModel;
        ...
        private void DoThreadWork()
        {
            while (m_bRun)
            {
                try
                {
                    if(m_cCurrentDevice ==null)
                    {
                        m_cCurrentDevice = m_qDeviceConfig.Dequeue();
                        m_iDataSize = m_cCurrentDevice.WordCount * 2 + 5;
                    }
                    ...
                    if (m_qDataS.Count > m_iDataSize)
                    {
                        ...
                        if(m_qDeviceConfig.Count>0)
                        {
                            m_cCurrentDevice = null;
                            m_iDataSize = 10000;
                        }
                        else
                        {
        ...
        private void DoDataRecieved(CMeterData cData)
        {
            try
            {
                if(m_cMeterDataRecorder!=null)
                {
                    if (m_cMeterDataRecorder.IsRunning)
                        m_cMeterDataRecorder.EnQue(cData);
                }
            }
            catch (System.Exception ex)
            {
