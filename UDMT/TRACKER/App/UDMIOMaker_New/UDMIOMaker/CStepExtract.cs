...
namespace UDMIOMaker
{
    public class CStepExtract
    {
        ...
        private static void SplitOneCoilStep(CStep cStep)
        {
            try
            {
                ...
                foreach (CCoil cCoil in cCoilS)
                {
                    ...
                    CreateNewStepRelation(cSplitStep, cStep, cRelatedContactS, sKey);
        ...
        private static void CopyPrevCoilRelation(CStep cOriginalStep, CContactS cRelatedContactS, int iStepIndex, string sStepKey, int iNextIndex, bool bNextIsCoil, CStep cSplitStep)
        {
            try
            {
                CCoil cTempCoil = FindCoilByIndex(cOriginalStep, iStepIndex);
                ...
                if (cTempCoil != null)
                {
                    if (cTempCoil.Relation.PrevContactS.Count != 0)
                    {
                        ...
                        {
                            CopyPrevContactRelation(cOriginalStep, cRelatedContactS, iPrevContact, sStepKey, iNextIndex, bNextIsCoil, cSplitStep);
                        }
                    }
                    ...
                    if (cTempCoil.Relation.PrevCoilS.Count != 0)
                    {
                        ...
                        {
                            CopyPrevCoilRelation(cOriginalStep, cRelatedContactS, iPrevCoil, sStepKey, iNextIndex, bNextIsCoil, cSplitStep);
                        }
                    }
                }
        ...
        private static void CreateNewStepRelation(CStep cSplitStep, CStep cOriginalStep, CContactS cRelatedContactS, string sStepKey)
        {
            try
            {
        ...
        private static void ConnectRelationToNext(CStep cRelatedStep, int iNextIndex, int iThisIndex, bool bIsNextCoil, bool bIsThisCoil)
        {
            try
            {
                if (bIsNextCoil)
                {
                    foreach (CCoil cTempCoil in cRelatedStep.CoilS)
                    {
                        if (iNextIndex == cTempCoil.StepIndex)
                        {
                            if (bIsThisCoil)
                            {
                ...
                {
                    foreach (CContact cTempContact in cRelatedStep.ContactS)
                    {
                        if (iNextIndex == cTempContact.StepIndex)
                        {
                            if (bIsThisCoil)
                            {
