...
namespace UDMIOMaker
{
    ...
    public class IOListExport
    {
        ...
        private void InputValues(Worksheet excelSheet, int row, string specifier, ABTag tag, List<ABTag> listTag)
        {
            ...
            if (FindTag.Count == 1)
            {
                excelSheet.Cells[row, IOListABSheetFormat.COlSYMBOLNAME] = FindTag[0].Description;
        ...
        private string GetSheetName(ABTag tag)
        {
            ...
            try
            {
                if (!tag.ABMODULE.Contains("_"))
                {
                    name = tag.ABMODULE.Replace("[", "(").Replace("]", ")").Replace(":", ";") + "-" + tag.ABINFO;
                    return name;
                }

                string[] addrss = tag.ABMODULE.Split('_');
