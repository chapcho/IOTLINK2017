...
namespace UDMTracker
{
	public partial class FrmStatisticsChartViewer : Form
	{
		...
		private void SetNavItemTag()
		{
			...
			{
				lstCMeanValueS.Add(m_lstRowS[i].GroupInfo.GroupKey, m_lstRowS[i].CycleInfo.Mean);
			}
			itemCycleMean.Tag = lstCMeanValueS;

			for (int i = 0; i < m_lstRowS.Count; i++)
			{
				lstCMinValueS.Add(m_lstRowS[i].GroupInfo.GroupKey, m_lstRowS[i].CycleInfo.Minimum);
			}
			itemCycleMin.Tag = lstCMinValueS;

			for (int i = 0; i < m_lstRowS.Count; i++)
			{
				lstCMaxValueS.Add(m_lstRowS[i].GroupInfo.GroupKey, m_lstRowS[i].CycleInfo.Maximum);
			}
			itemCycleMax.Tag = lstCMaxValueS;

			for (int i = 0; i < m_lstRowS.Count; i++)
			{
				lstStdValueS.Add(m_lstRowS[i].GroupInfo.GroupKey, m_lstRowS[i].CycleInfo.StandardDev);
			}
			itemCycleStd.Tag = lstStdValueS;

			for (int i = 0; i < m_lstRowS.Count; i++)
			{
				lstCpValueS.Add(m_lstRowS[i].GroupInfo.GroupKey, m_lstRowS[i].CycleInfo.Cp);
			}
			itemCycleCp.Tag = lstCpValueS;

			for (int i = 0; i < m_lstRowS.Count; i++)
			{
				lstCpkValueS.Add(m_lstRowS[i].GroupInfo.GroupKey, m_lstRowS[i].CycleInfo.Cpk);
			}
