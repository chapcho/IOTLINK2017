	...
	public partial class UCOPCProperty : UserControl
	{
        ...
        private void cmbServerList_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (m_cOPCConfig.LsOpc)
            {
                ...
                if (m_cOPCConfig.ChannelDevice != "")
                {
                    cmbChannelList.Properties.Items.Add(m_cOPCConfig.ChannelDevice);
                }
                else
                {
                    FrmInputDialog frmInput = new FrmInputDialog("OPC Device", "수집할 OPC Device를 입력하세요");
            ...
            {
                List<string> lstDevice = GetOPCDeviceList(m_sSelectedServer);
            }
