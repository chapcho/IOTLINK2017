...
namespace UDM.Monitor.Plc.Source.OPC
{
	public class COPCItemS : Dictionary<string, COPCItem>, IDisposable
	{
		...
		protected OPCAutomation.OPCItems m_exOPCItemS = null;
		...
		public bool AddItemS(List<CTag> lstTag, bool bLsOpc)
		{
			...
			for (int i = 0; i < lstTag.Count; i++)
			{
				...
				{
                    cItem = new COPCItem(cTag, bLsOpc);
		...
		public List<COPCItem> AddInstantItemS(List<CTag> lstTag, bool bLsOpc)
		{
			if (m_bReady == false)
				return null;
		...
		private bool AddOPCItemS(List<COPCItem> lstItem)
		{
			...
			try
			{
				...
				Array arServerHandles = null;
				Array arResult = null;

				m_exOPCItemS.AddItems(lstItem.Count, ref arItemKeys, ref arClientHandles, out arServerHandles, out arResult);
				if (arResult != null)
				{	
					int iResult = 0;
					COPCItem cItem;
					for (int i = 0; i < lstItem.Count; i++)
					{
						...
						if (iResult == 0)
						{
							cItem.ServerHandle = (int)arServerHandles.GetValue(i + 1);
		...
		private bool RemoveOPCItemS(List<COPCItem> lstItem)
		{
			...
			try
			{
				...
				m_exOPCItemS.Remove(lstItem.Count, ref arServerHandles, out arResult);
				if (arResult != null)
				{
					int iResult = 0;
					COPCItem cItem;
