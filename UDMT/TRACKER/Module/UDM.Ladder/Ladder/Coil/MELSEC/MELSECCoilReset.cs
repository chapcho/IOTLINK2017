...
{
    public static class MELSECCoilReset
    {
        ...
        public static void DrawAdditional(Graphics graphics, Point point, bool bValue, string strTime, CCoil cCoil, Font font, Brush brush, CTimeLogS cLogS)
        {
            try
            {
                ...
                float segment = w / 8f; // (4 cells * 2 segments)

                Brush brushInfo = bValue ? new SolidBrush(Color.Yellow) : brush;

                // Size font address
                SizeF sizeFont = graphics.MeasureString(strAddress, font);
                graphics.DrawString(strAddress, font, brushInfo, new PointF(point.X + 3 * segment, point.Y - (sizeFont.Height / 2f)), stringFormat);

                // Size font RST
                sizeFont = graphics.MeasureString("RST", font);
                graphics.DrawString("RST", font, brushInfo, new PointF(point.X + 1 * segment, point.Y - (sizeFont.Height / 2f)), stringFormat);
                ...
                graphics.DrawString(strTime, fontTime, brush, new RectangleF(point.X - 1 * segment, point.Y + levelHeight, w, nWrapBoxHeight), stringFormat);
                levelHeight += sizeFont.Height;

                // String symbol (comment)
                fontComment = new Font(font.SystemFontName, (float)(0.75 * font.Size));
                graphics.DrawString(strDescription, fontComment, brush, new RectangleF(point.X - 1 * segment, point.Y + levelHeight, w, nWrapBoxHeight), stringFormat);

                //Draw Word Value
                if (cLogS != null && cLogS.Count > 0)
                {
                    Font fontWordValue = null;
                    CTag cTag = cCoil.RefTagS.GetFirstTag(strAddress);

                    if(cTag.DataType != EMDataType.Bool)
                    {
                        foreach (CTimeLog cLog in cLogS)
                        {
                            if (cLog.Key == cTag.Key)
                            {
                                levelHeight = -20F;
                                fontWordValue = new Font(font.SystemFontName, (int)(0.8 * font.Size), FontStyle.Bold);
                                graphics.DrawString(cLog.Value.ToString(), fontWordValue, new SolidBrush(Color.Fuchsia), new RectangleF(point.X - 1 * segment, point.Y + levelHeight, w, nWrapBoxHeight), stringFormat);
                            }
                        }
                    }
                }
