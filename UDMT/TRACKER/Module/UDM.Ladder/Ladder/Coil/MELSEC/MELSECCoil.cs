...
{
    public class MELSECCoil : CLadderItem, ILadderItemCoil
    {
...
#if CUSTOM_PAJU
                // NOTHING ... (just to skip additional draw)
#else

                    if (m_cCoil.Command == "SET")
                        MELSECCoilSet.DrawAdditional(graphics, point, m_bValue, m_dtTime.ToString(m_sFormatDate), m_cCoil.RefTagS.GetBaseAddress(), m_cCoil.RefTagS.GetBaseDescription(), m_font, m_brushText);
                    else if (m_cCoil.Command == "RST")
                        MELSECCoilReset.DrawAdditional(graphics, point, m_bValue, m_dtTime.ToString(m_sFormatDate), m_cCoil.RefTagS.GetBaseAddress(), m_cCoil.RefTagS.GetBaseDescription(), m_font, m_brushText);
                    else if (m_cCoil.CoilType == EMCoilType.Special)
                        MELSECCoilSpecial.DrawAdditional(graphics, point, m_bValue, m_dtTime.ToString(m_sFormatDate), m_cCoil, m_font, m_brushText);
                    else
                        MELSECCoilNormal.DrawAdditional(graphics, point, m_bValue, m_dtTime.ToString(m_sFormatDate), m_cCoil.RefTagS.GetBaseAddress(), m_cCoil.RefTagS.GetBaseDescription(), m_font, m_brushText);
#endif
            }
        ...
        private void Update()
        {
            ...
            ClearResources();
                ...
                else if (m_cCoil.CoilType == EMCoilType.Special) { CoilSpecial(); }
                else { CoilNormal(); }
#endif
        }
