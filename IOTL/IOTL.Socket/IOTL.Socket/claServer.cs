    ...
    public class claServer : AppServer<claClientSession, BinaryRequestInfo>
    {
        ...
        protected override void ExecuteCommand(claClientSession session, BinaryRequestInfo requestInfo)
        {
            ...
            LocalMessageEventArgs e
                = new LocalMessageEventArgs(session.SessionID, requestInfo.Key, requestInfo.Body);
        ...
        /// <returns></returns>
        public bool SendDataToSocketClient(string socketClientID, byte[] sendData)
        {
        ...
        public bool SendMsgToClient(string userId, string message)
        {
            bool bRet = false;
            foreach(claClientSession insUserTemp in this.GetAllSessions())
            {
                if(insUserTemp.UserID.Equals(userId))
                {
                    insUserTemp.SendMsg_User(message);
                    bRet = true;
                    break;
                }
            }
        ...
        public bool SendBytesToSocketClient(string sessionID, byte[] sendData )
        {
            bool bRet = false;
            foreach (claClientSession insUserTemp in this.GetAllSessions())
            {
                if (insUserTemp.SessionID.Equals(sessionID))
                {
